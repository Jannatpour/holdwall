...
exports.CitationAwareSelector = void 0;
const logger_1 = require("@/lib/logging/logger");
/**
 * Citation-Aware Chunk Selector
 */
class CitationAwareSelector {
    /**
     * Select chunks with citation awareness
     */
    selectChunks(chunks, config) {
        ...
        if (ensureCoverage) {
            const chunksWithStrongCitations = chunksWithCitations.filter((c) => c.hasStrongLinks);
     ...
     */
    calculateCitationStrength(evidence) {
    ...
    calculateEvidenceQuality(evidence) {
        ...
        let factors = 0;
        // Content quality
        const hasContent = Boolean(evidence.content?.normalized || evidence.content?.raw || evidence.metadata?.title);
        if (hasContent) {
            quality += 0.3;
            factors++;
        }
     ...
     */
    ensureCitationCoverage(chunks, minCoverage = 3) {
        ...
        logger_1.logger.warn("Citation coverage insufficient", {
...
exports.CitationAwareSelector = CitationAwareSelector;
