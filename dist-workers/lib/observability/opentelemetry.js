...
exports.otelTracer = exports.OpenTelemetryTracer = void 0;
...
class OpenTelemetryTracer {
    ...
    endSpan(spanId, status = "OK", error) {
        ...
        logger_1.logger.info("span", {
            ...
            spanId: span.spanId,
            name: span.name,
            duration: span.duration / 1000000, // Convert to milliseconds
    ...
    async exportSpan(span) {
        ...
        Promise.all(this.exporters.map((exporter) => exporter([span]).catch((error) => {
            logger_1.logger.warn("Span export failed", { error: error.message });
        }))).catch(() => {
...
exports.otelTracer = new OpenTelemetryTracer();
/**
 * Register default exporters
 */
if (process.env.OTEL_EXPORTER_OTLP_ENDPOINT) {
    // OTLP exporter (OpenTelemetry Protocol)
    exports.otelTracer.registerExporter(async (spans) => {
        ...
        try {
            ...
            logger_1.logger.warn("OTLP export error", { error: error.message });
            throw error;
        }
    });
}
...
if (process.env.JAEGER_ENDPOINT) {
    exports.otelTracer.registerExporter(async (spans) => {
        ...
        logger_1.logger.info("Jaeger export", {
            endpoint,
