...
class ClaimTemplateMatcher {
     ...
     */
    async match(evidenceId, tenantId) {
        try {
            ...
            const templates = await client_1.db.claimTemplate.findMany({
                where: { tenantId },
            ...
            await client_1.db.claimTemplate.update({
            ...
            logger_1.logger.error("Failed to match claim template", {
     ...
     */
    async calculateSimilarity(content, template) {
        try {
            ...
            return this.embeddingService.cosineSimilarity(contentEmbedding.vector, templateEmbedding.vector);
        }
        catch (error) {
            logger_1.logger.warn("Failed to calculate semantic similarity, using text similarity", {
                error: error instanceof Error ? error.message : String(error),
            });
     ...
     */
    async createTemplate(tenantId, canonicalText, variants = []) {
        const template = await client_1.db.claimTemplate.create({
            data: {
                tenantId,
...
exports.ClaimTemplateMatcher = ClaimTemplateMatcher;
