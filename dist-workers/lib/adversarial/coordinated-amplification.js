...
exports.CoordinatedAmplificationDetector = void 0;
const client_1 = require("@/lib/db/client");
const logger_1 = require("@/lib/logging/logger");
const embeddings_1 = require("@/lib/vector/embeddings");
class CoordinatedAmplificationDetector {
    constructor() {
    ...
    async detect(evidenceId, tenantId) {
        try {
            ...
            const relatedEvidence = await client_1.db.evidence.findMany({
                where: {
                    tenantId,
            ...
            logger_1.logger.error("Failed to detect coordinated amplification", {
     ...
     */
    analyzeTiming(evidence, relatedEvidence) {
     ...
     */
    async analyzeContentSimilarity(evidence, relatedEvidence) {
        try {
            ...
            for (const related of relatedEvidence.slice(0, 20)) {
                ...
                const similarity = this.embeddingService.cosineSimilarity(evidenceEmbedding.vector, relatedEmbedding.vector);
            ...
            logger_1.logger.warn("Failed to analyze content similarity", {
                error: error instanceof Error ? error.message : String(error),
            });
            return 0;
        }
     ...
     */
    analyzeNetworkPatterns(evidence, relatedEvidence) {
...
exports.CoordinatedAmplificationDetector = CoordinatedAmplificationDetector;
