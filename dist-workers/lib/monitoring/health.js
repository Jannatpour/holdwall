...
const staging_parity_1 = require("@/lib/environment/staging-parity");
async function checkHealth() {
    ...
    try {
        ...
        await withTimeout(client_1.db.$queryRaw `SELECT 1`, 4000, "Database health check");
        checks.database = "ok";
    }
    catch (error) {
        const { logger } = await Promise.resolve().then(() => __importStar(require("@/lib/logging/logger")));
        ...
        try {
            ...
            if (supabaseUrl && serviceRoleKey) {
                ...
                const resp = await withTimeout(fetch(`${baseUrl}/rest/v1/User?select=id&limit=1`, {
                    method: "GET",
                    headers: {
                ...
                }), 1500, "Supabase REST health check");
                if (resp.ok) {
                    checks.database = "ok";
                }
                else {
    ...
    if (typeof process.memoryUsage === "function") {
        ...
        try {
            const v8 = await Promise.resolve().then(() => __importStar(require("node:v8")));
    ...
    if (process.env.HEALTH_INCLUDE_PARITY === "true") {
        try {
            await withTimeout((0, staging_parity_1.checkStagingParity)(), 1500, "Staging parity check");
        }
        catch (error) {
            const { logger: healthLogger } = await Promise.resolve().then(() => __importStar(require("@/lib/logging/logger")));
