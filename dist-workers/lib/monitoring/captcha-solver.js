...
class CaptchaSolver {
     ...
     */
    async solveWith2Captcha(imageBase64) {
        ...
        for (let i = 0; i < maxAttempts; i++) {
            ...
            const resultResponse = await fetch(`http://2captcha.com/res.php?key=${apiKey}&action=get&id=${captchaId}&json=1`);
     ...
     */
    async solveRecaptchaWith2Captcha(siteKey, pageUrl) {
        const apiKey = this.apiKeys.get("2captcha");
        if (!apiKey) {
            throw new Error("2Captcha API key not configured");
        }
     ...
     */
    async solveWithAntiCaptcha(imageBase64) {
     ...
     */
    async solveRecaptchaWithAntiCaptcha(siteKey, pageUrl) {
        const apiKey = this.apiKeys.get("anticaptcha");
        if (!apiKey) {
            throw new Error("AntiCaptcha API key not configured");
        }
     ...
     */
    async solveWithDeathByCaptcha(imageBase64) {
     ...
     */
    async solveRecaptchaWithDeathByCaptcha(siteKey, pageUrl) {
        const apiKey = this.apiKeys.get("deathbycaptcha");
        if (!apiKey) {
            throw new Error("DeathByCaptcha API key not configured");
        }
