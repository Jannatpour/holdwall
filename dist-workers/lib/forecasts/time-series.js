...
exports.ProphetModel = exports.ARIMAModel = void 0;
/**
...
class ARIMAModel {
     ...
     */
    async forecast(data, horizon, params) {
        ...
        const forecastedDiff = this.forecastARMA(stationary, model.coefficients, horizon, p, q);
        ...
        const residualStd = Math.sqrt(model.residuals.reduce((sum, r) => sum + r * r, 0) / model.residuals.length);
     ...
     * Estimate MA coefficients using innovation algorithm
     */
    estimateMACoefficients(data, arCoeffs, q) {
        if (q === 0) {
            return [];
        }
     ...
     * Calculate residuals
     */
    calculateResiduals(data, arCoeffs, maCoeffs) {
        const residuals = [];
        const mean = data.reduce((sum, val) => sum + val, 0) / data.length;
     ...
     */
    forecastARMA(data, coefficients, horizon, p, q) {
        const forecasted = [];
        const mean = data.reduce((sum, val) => sum + val, 0) / data.length;
...
class ProphetModel {
     ...
     */
    async forecast(data, horizon, options) {
        if (data.length < 10) {
     ...
     * Forecast trend
     */
    forecastTrend(trend, changepoints, horizon) {
        const forecasted = [];
     ...
     */
    fitSeasonal(data, trend, options) {
        const seasonal = [];
     ...
     */
    forecastSeasonal(seasonal, horizon, options) {
        const forecasted = [];
    ...
    detectChangepoints(data) {
        ...
        for (let i = window; i < data.length - window; i++) {
            ...
            const pooledStd = Math.sqrt((before.reduce((sum, val) => sum + Math.pow(val - meanBefore, 2), 0) +
