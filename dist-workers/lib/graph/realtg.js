...
exports.ReaLTG = void 0;
class ReaLTG {
    constructor() {
        this.openaiApiKey = null;
        this.linkHistory = new Map(); // "from-to" -> history
        this.openaiApiKey = process.env.OPENAI_API_KEY || null;
    }
    /**
     * Forecast links with explainable reasoning
     */
    async forecast(query, nodes, existingEdges, timeWindow = 7 // days
    ) {
        const forecasts = [];
        ...
        for (let i = 0; i < nodes.length; i++) {
            for (let j = i + 1; j < nodes.length; j++) {
                ...
                const existingEdge = existingEdges.find(e => (e.from_node_id === fromNode.node_id && e.to_node_id === toNode.node_id) ||
                    (e.from_node_id === toNode.node_id && e.to_node_id === fromNode.node_id));
                if (existingEdge) {
                    // Forecast evolution of existing edge
                    const forecast = await this.forecastExistingLink(existingEdge, fromNode, toNode, timeWindow);
                    forecasts.push(forecast);
                }
                else {
                    // Forecast new link formation
                    const forecast = await this.forecastNewLink(fromNode, toNode, timeWindow);
     ...
     */
    async forecastExistingLink(edge, fromNode, toNode, timeWindow) {
     ...
     * Forecast new link formation
     */
    async forecastNewLink(fromNode, toNode, timeWindow) {
     ...
     * Generate reasoning
     */
    async generateReasoning(query, forecasts, nodes) {
