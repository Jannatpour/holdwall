...
class CODEN {
     ...
     * Record node state
     */
    recordState(node, timestamp = new Date().toISOString()) {
     ...
     */
    predict(node, edges, timeWindow = 7 // days
    ) {
        ...
        for (let i = 1; i <= (timeWindow * 24) / hoursPerPrediction; i++) {
            ...
            const predicted = this.predictAtTime(node, edges, recent, predictionTime, trend);
     ...
     */
    predictAtTime(node, edges, history, targetTime, trend) {
        ...
        const connectedEdges = edges.filter(e => e.from_node_id === node.node_id || e.to_node_id === node.node_id);
