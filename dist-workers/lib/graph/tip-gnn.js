...
exports.TIPGNN = void 0;
class TIPGNN {
    constructor() {
        this.transitions = new Map(); // nodeId -> transitions
    }
    /**
     * Record transition
     */
    recordTransition(nodeId, transition) {
     ...
     */
    predict(node, neighbors, timeSteps = 1) {
        const currentState = {
        ...
        const transitionProbability = this.calculateTransitionProbability(currentState, predictedState, transitionPattern);
        return {
            nodeId: node.node_id,
            currentState,
            predictedState,
    ...
    analyzeTransitions(transitions) {
        ...
        const changes = transitions.map(t => (t.toState.trust - t.fromState.trust) + (t.toState.decisiveness - t.fromState.decisiveness));
     ...
     */
    calculateInfluence(neighbor, edge, transitionPattern) {
     ...
     * Calculate transition probability
     */
    calculateTransitionProbability(current, predicted, transitionPattern) {
