...
async function seedDatabase() {
    ...
    const tenant = await client_1.db.tenant.upsert({
        where: { slug: "default" },
        update: {},
        create: {
            name: "Default Tenant",
    ...
    for (const sourceType of sourceTypes) {
        await client_1.db.sourcePolicy.upsert({
            where: {
                tenantId_sourceType: {
                    tenantId: tenant.id,
                    sourceType,
    ...
    try {
        ...
        if (adminCheck.rows.length > 0) {
            ...
            await pool.query('UPDATE "User" SET "passwordHash" = $1, name = $2, role = $3, "tenantId" = $4, "updatedAt" = NOW() WHERE email = $5', [adminPasswordHash, "Admin User", "ADMIN", tenant.id, "admin@holdwall.com"]);
        }
        else {
            // Create new admin user
            await pool.query(`INSERT INTO "User" (id, email, name, "passwordHash", role, "tenantId", "createdAt", "updatedAt")
         VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW())`, [`user-${Date.now()}`, "admin@holdwall.com", "Admin User", adminPasswordHash, "ADMIN", tenant.id]);
        }
        ...
        if (userCheck.rows.length > 0) {
            ...
            await pool.query('UPDATE "User" SET "passwordHash" = $1, name = $2, role = $3, "tenantId" = $4, "updatedAt" = NOW() WHERE email = $5', [userPasswordHash, "Test User", "USER", tenant.id, "user@holdwall.com"]);
        }
        else {
            // Create new user
            await pool.query(`INSERT INTO "User" (id, email, name, "passwordHash", role, "tenantId", "createdAt", "updatedAt")
         VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW())`, [`user-${Date.now() + 1}`, "user@holdwall.com", "Test User", userPasswordHash, "USER", tenant.id]);
        }
        // Verify password hashes are set and valid
        const adminVerify = await pool.query('SELECT email, "passwordHash", LENGTH("passwordHash") as hash_len FROM "User" WHERE email = $1', ["admin@holdwall.com"]);
        const userVerify = await pool.query('SELECT email, "passwordHash", LENGTH("passwordHash") as hash_len FROM "User" WHERE email = $1', ["user@holdwall.com"]);
        // Verify admin password
        let adminValid = false;
        if (adminVerify.rows[0]?.passwordHash && adminVerify.rows[0].hash_len >= 60) {
            try {
                adminValid = await bcryptjs_1.default.compare("admin123", adminVerify.rows[0].passwordHash.trim());
            }
            catch (e) {
        ...
        if (userVerify.rows[0]?.passwordHash && userVerify.rows[0].hash_len >= 60) {
            try {
                userValid = await bcryptjs_1.default.compare("user123", userVerify.rows[0].passwordHash.trim());
            }
            catch (e) {
