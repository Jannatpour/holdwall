...
class LLMProvider {
     ...
     */
    async callStream(request, options) {
        const provider = this.detectProvider(request.model);
        switch (provider) {
            case "openai":
    ...
    async callOpenAIStream(request, options) {
                ...
                for (const line of lines) {
                    ...
                    try {
                        ...
                        if (toolCalls && Array.isArray(toolCalls)) {
                            ...
                            logger_1.logger.debug("Tool calls in stream", { toolCalls });
                        }
                        if (delta) {
                            text += delta;
    ...
    async callAnthropicStream(request, options) {
    ...
    async callOpenAI(request) {
        ...
        try {
            ...
            if (toolCalls && Array.isArray(toolCalls) && toolCalls.length > 0) {
                ...
                logger_1.logger.debug("Function calling response received", { toolCalls });
    ...
    async callAnthropic(request) {
        ...
        try {
            ...
            if (!response.ok) {
                const error = await response.json().catch(() => ({ error: { message: response.statusText } }));
                logger_1.logger.error("Anthropic API call failed", {
            ...
            if (toolUse) {
                logger_1.logger.debug("Anthropic tool use response received", { toolUse });
    ...
    async callGeneric(request) {
        throw new Error(`Unsupported model/provider for model "${request.model}". Configure a supported provider (OpenAI or Anthropic) or use a supported model name.`);
    }
    /**
