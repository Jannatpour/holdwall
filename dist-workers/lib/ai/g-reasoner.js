...
exports.gReasoner = exports.GReasoner = void 0;
...
class GReasoner {
     ...
     */
    async reason(input, tenantId) {
        const startTime = Date.now();
        try {
            logger_1.logger.info("G-reasoner reasoning started", {
                query: input.query,
            ...
            const graphPath = await this.traverseGraph(queryUnderstanding, input.nodes, input.edges);
            // Step 4: Unified reasoning - combine graph and language for reasoning
            const reasoning = await this.unifiedReasoning(input.query, graphEncoding, queryUnderstanding, graphPath, tenantId);
            // Step 5: Evidence extraction - extract supporting evidence
            const evidence = await this.extractEvidence(graphPath, input.nodes, queryUnderstanding);
            const result = {
            ...
            metrics_1.metrics.increment("g_reasoner.reasoning");
            metrics_1.metrics.observe("g_reasoner.latency", latencyMs);
            metrics_1.metrics.gauge("g_reasoner.confidence", reasoning.confidence);
            logger_1.logger.info("G-reasoner reasoning completed", {
                confidence: reasoning.confidence,
            ...
            logger_1.logger.error("G-reasoner reasoning failed", {
...
${edgeDescriptions}`;
     ...
     */
    async understandQuery(query, graphEncoding, tenantId) {
...
Return JSON:
{
  ...
  "graphRelevance": 0.0-1.0
        ...
        try {
            ...
            logger_1.logger.warn("Query understanding failed", { error });
        }
        // Fallback
        return {
            intent: query,
     ...
     */
    async traverseGraph(queryUnderstanding, nodes, edges) {
        const path = [];
        ...
        const startNodes = nodes.filter((node) => {
            ...
            return queryUnderstanding.entities.some((entity) => content.includes(entity.toLowerCase()));
        });
     ...
     */
    async unifiedReasoning(query, graphEncoding, queryUnderstanding, graphPath, tenantId) {
...
Return JSON:
{
  ...
  "confidence": 0.0-1.0
        ...
        try {
            ...
            logger_1.logger.warn("Unified reasoning failed", { error });
        }
        // Fallback
        return {
     ...
     */
    async extractEvidence(graphPath, nodes, queryUnderstanding) {
        const nodeMap = new Map(nodes.map((n) => [n.id, n]));
        const evidence = [];
        for (const pathNode of graphPath) {
            ...
            if (node) {
                ...
                const content = node.content?.toLowerCase() || "";
                const relevance = queryUnderstanding.entities.some((entity) => content.includes(entity.toLowerCase()))
                    ? 0.8
                    : 0.5;
                evidence.push({
                    nodeId: node.id,
...
exports.GReasoner = GReasoner;
exports.gReasoner = new GReasoner();
