...
class CoRAG {
    ...
    async execute(query, tenantId, options) {
        ...
        for (let step = 1; step <= maxSteps; step++) {
            ...
            const reasoning = await this.reasonAboutRetrieval(query, currentQuery, context, steps);
            // Determine if we need another step
            const needsMore = await this.needsMoreRetrieval(query, context, reasoning, minConfidence);
            steps.push({
                step,
                query: currentQuery,
     ...
     */
    async reasonAboutRetrieval(originalQuery, currentQuery, context, previousSteps) {
     ...
     */
    async needsMoreRetrieval(originalQuery, context, reasoning, minConfidence) {
     ...
     * Generate next query to fill gaps
     */
    async generateNextQuery(originalQuery, gaps) {
     ...
     * Generate final answer
     */
    async generateFinalAnswer(query, context, steps) {
     ...
     */
    calculateConfidence(steps, finalContext) {
