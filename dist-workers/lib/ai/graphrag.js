...
const leiden_clustering_1 = require("./leiden-clustering");
class GraphRAG {
    constructor() {
        this.knowledgeGraph = {
            entities: new Map(),
        ...
        this.evidenceStore = new Map(); // entityId -> Evidence[]
        this.embeddings = new embeddings_1.EmbeddingService();
        this.llmProvider = new providers_1.LLMProvider();
    }
    /**
     * Build knowledge graph from evidence
     */
    async buildKnowledgeGraph(evidence) {
        ...
        for (const rel of relationships) {
            ...
            if (fromNode)
                fromNode.neighbors.add(rel.to);
            if (toNode)
                toNode.neighbors.add(rel.from);
        }
        // Perform Leiden clustering
        const clusteringResult = await leiden_clustering_1.leidenClustering.cluster(graphNodes, graphEdges, {
            resolution: 1.0,
            hierarchical: true,
        });
        this.knowledgeGraph = {
     ...
     */
    async extractEntities(evidence) {
...
[
  {
     ...
     */
    async extractRelationships(evidence, entities) {
...
[
  {
    ...
    "evidence": "brief text evidence for this relationship"
        ...
        if (relationships.length === 0 && entities.length >= 2) {
            ...
            const entityNames = new Map(entities.map(e => [e.id, (e.properties.name || e.id).toLowerCase()]));
     ...
     */
    async findRelevantEntities(queryEmbedding, maxResults) {
        const entities = Array.from(this.knowledgeGraph.entities.entries());
        const scored = await Promise.all(entities.map(async ([entityId, entity]) => {
     ...
     */
    async retrieveEvidence(entityIds) {
     ...
     */
    async generateAnswer(query, evidence) {
        ...
        try {
            ...
            const sources = evidence.slice(0, 5).map((ev, idx) => ({
