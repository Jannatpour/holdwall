...
exports.reflectRetryReward = exports.ReflectRetryReward = void 0;
const logger_1 = require("@/lib/logging/logger");
const metrics_1 = require("@/lib/observability/metrics");
const score_rl_1 = require("./score-rl");
/**
 * Reflect-Retry-Reward Framework
 *
...
class ReflectRetryReward {
     ...
     */
    async reflect(agentId, operationId, outcome, performance, context) {
        const reflectionId = `reflection-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`;
        try {
            logger_1.logger.info("Reflection started", {
                reflectionId,
                agentId,
                operationId,
                outcome,
            ...
            // Generate insights
            const insights = await this.generateInsights(agentId, operationId, outcome, performance, context);
            const reflection = {
                id: reflectionId,
                agentId,
                operationId,
            ...
            metrics_1.metrics.increment("reflect_retry_reward.reflections", {
                agent_id: agentId,
                outcome,
            });
            metrics_1.metrics.gauge("reflect_retry_reward.performance", performance, { agent_id: agentId });
            ...
            logger_1.logger.error("Reflection failed", {
                reflectionId,
     ...
     */
    async generateRetryStrategy(reflection) {
        const strategyId = `strategy-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`;
        try {
            logger_1.logger.info("Retry strategy generation started", {
                strategyId,
            ...
            metrics_1.metrics.increment("reflect_retry_reward.strategies_generated");
            metrics_1.metrics.gauge("reflect_retry_reward.strategy_confidence", strategy.confidence);
            logger_1.logger.info("Retry strategy generated", {
                strategyId,
            ...
            logger_1.logger.error("Retry strategy generation failed", {
                strategyId,
                error: error instanceof Error ? error.message : String(error),
            });
     ...
     */
    async reward(strategyId, value, source) {
        ...
        if (strategy && value > 0.7) {
            ...
            this.successfulPatterns.set(patternKey, (this.successfulPatterns.get(patternKey) || 0) + value);
        }
        // Update SCoRe RL with reward
        if (strategy) {
            const reflection = this.reflections.get(strategy.reflectionId);
            if (reflection) {
                await score_rl_1.scoreRL.learnFromOutcome({
        ...
        metrics_1.metrics.increment("reflect_retry_reward.rewards", { source });
        metrics_1.metrics.gauge("reflect_retry_reward.reward_value", value);
        logger_1.logger.info("Reward recorded", {
            rewardId,
            strategyId,
            value,
            source,
     ...
     */
    async generateInsights(agentId, operationId, outcome, performance, context) {
        const insights = [];
     ...
     */
    getSuccessfulPatterns(limit = 10) {
     ...
     */
    getReflectionHistory(agentId, limit = 50) {
...
exports.ReflectRetryReward = ReflectRetryReward;
exports.reflectRetryReward = new ReflectRetryReward();
