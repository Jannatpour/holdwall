...
exports.leidenClustering = exports.LeidenClustering = void 0;
...
class LeidenClustering {
    constructor() {
        this.resolution = 1.0; // Resolution parameter (higher = more communities)
        this.maxIterations = 100;
        this.minImprovement = 0.0001;
    }
    /**
     * Perform Leiden clustering on a graph
     */
    async cluster(nodes, edges, options) {
        ...
        logger_1.logger.info("Leiden clustering started", {
        ...
        while (improved && iterations < maxIterations) {
            ...
            logger_1.logger.debug("Leiden iteration", {
        ...
        logger_1.logger.info("Leiden clustering completed", {
     ...
     * Local moving phase: move nodes to best community
     */
    async localMoving(partition, nodes, edges, adjacency, resolution) {
        ...
        for (const nodeId of nodeIds) {
            ...
            const candidateCommunities = new Set([currentCommunity]);
            for (const neighborId of neighbors.keys()) {
                candidateCommunities.add(newPartition.get(neighborId));
            }
            for (const candidateCommunity of candidateCommunities) {
                const gain = this.calculateModularityGain(nodeId, candidateCommunity, currentCommunity, nodes, edges, adjacency, newPartition, resolution);
                if (gain > bestGain) {
                    bestGain = gain;
     ...
     */
    async refine(partition, nodes, edges, adjacency, resolution) {
        const newPartition = new Map(partition);
     ...
     */
    async trySplit(nodes, allNodes, edges, adjacency, resolution) {
        if (nodes.length <= 2) {
            return null;
        }
        // Create subgraph
        ...
        for (const nodeId of nodes) {
            ...
            if (node) {
                subgraphNodes.set(nodeId, {
                    id: nodeId,
                    neighbors: new Set(Array.from(node.neighbors).filter((n) => nodes.includes(n))),
                });
            }
        }
        for (const edge of edges) {
        ...
        let subgraphPartition = new Map();
        for (const nodeId of nodes) {
            subgraphPartition.set(nodeId, nodeId);
        }
        // Run local moving on subgraph
        subgraphPartition = await this.localMoving(subgraphPartition, subgraphNodes, subgraphEdges, subgraphAdjacency, resolution);
        // Check if split improves modularity
        const originalModularity = this.calculateModularity(new Map(nodes.map((n) => [n, nodes[0]])), // All in one community
        subgraphNodes, subgraphEdges, subgraphAdjacency, resolution);
        const splitModularity = this.calculateModularity(subgraphPartition, subgraphNodes, subgraphEdges, subgraphAdjacency, resolution);
     ...
     */
    calculateModularity(partition, nodes, edges, adjacency, resolution) {
        const communities = new Set(partition.values());
        let modularity = 0;
        let totalWeight = 0;
     ...
     */
    calculateModularityGain(nodeId, targetCommunity, currentCommunity, nodes, edges, adjacency, partition, resolution) {
     ...
     * Build communities from partition
     */
    buildCommunities(partition, nodes, edges, adjacency) {
     ...
     */
    async buildHierarchy(communities, nodes, edges, adjacency, resolution) {
        ...
        while (currentCommunities.length > 1 && currentLevel < 5) {
            ...
            for (const edge of edges) {
                const fromCommunity = Array.from(communities.entries()).find(([_, c]) => c.nodes.has(edge.from))?.[1];
                const toCommunity = Array.from(communities.entries()).find(([_, c]) => c.nodes.has(edge.to))?.[1];
                if (fromCommunity && toCommunity && fromCommunity.id !== toCommunity.id) {
                    const existingEdge = superEdges.find((e) => e.from === fromCommunity.id && e.to === toCommunity.id);
                    if (existingEdge) {
            ...
            let superPartition = new Map();
            for (const community of currentCommunities) {
                superPartition.set(community.id, community.id);
            }
            superPartition = await this.localMoving(superPartition, superNodes, superEdges, superAdjacency, resolution * 0.5 // Lower resolution for higher levels
            );
            // Build next level communities
            const nextLevelCommunities = new Map();
...
exports.LeidenClustering = LeidenClustering;
exports.leidenClustering = new LeidenClustering();
