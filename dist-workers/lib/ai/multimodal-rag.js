...
exports.MultimodalRAG = void 0;
const multimodal_extractor_1 = require("../monitoring/multimodal-extractor");
class MultimodalRAG {
    ...
    async execute(query, tenantId, options) {
        ...
        return {
            ...
            sources: multimodalEvidence.map(e => e.metadata?.source || "").filter(Boolean),
        };
    }
    /**
     * Extract image evidence
     */
    async extractImageEvidence(evidence) {
        const imageEvidence = [];
     ...
     */
    async extractVideoEvidence(evidence) {
        const videoEvidence = [];
        for (const ev of evidence) {
            const content = typeof ev.content === "string"
                ? ev.content
     ...
     * Generate answer from multimodal evidence
     */
    async generateMultimodalAnswer(query, evidence) {
...
exports.MultimodalRAG = MultimodalRAG;
