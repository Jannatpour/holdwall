...
class EmbeddingService {
    ...
    async embed(text, model = "text-embedding-3-small") {
        ...
        if (process.env.NODE_ENV === "production") {
            throw new Error("Embedding providers unavailable (check OPENAI_API_KEY / VOYAGE_API_KEY / GOOGLE_API_KEY and billing/quota).");
        }
     ...
     */
    async embedWithOpenAI(text, model) {
     ...
     */
    async embedWithCohere(text, model) {
     ...
     */
    findSimilar(queryEmbedding, candidateEmbeddings, topK = 10) {
