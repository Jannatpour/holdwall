...
export class MultimodalExtractor {
   ...
   */
  async extractTextFromImageWithOpenAI(
    imageUrl: string,
    options?: {
      model?: string;
    }
    ...
    try {
      ...
      throw new Error(
        `OpenAI Vision extraction failed: ${error instanceof Error ? error.message : "Unknown error"}`
      );
    }
  }
  ...
  async extractTranscriptFromVideo(
    ...
    try {
      ...
      if (typeof videoUrl === "string") {
        ...
        file = new Blob([videoUrl as BlobPart]);
      }
      ...
      let transcript = "";
      let segments: VideoExtractionResult["segments"] = undefined;
      ...
      throw new Error(
        `Video transcription failed: ${error instanceof Error ? error.message : "Unknown error"}`
      );
    }
  }
  ...
  async extractTextFromImages(
    ...
    for (const imageUrl of imageUrls) {
      try {
        let result: ImageExtractionResult;
        ...
        } as ImageExtractionResult & { error?: string });
      }
    }
   ...
   */
  private async extractVideoMetadataFromElement(
    url: string
  ): Promise<{
    duration?: number;
    width?: number;
