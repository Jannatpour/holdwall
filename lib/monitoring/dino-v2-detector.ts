...
export interface DINOv2Features {
  patches: Array<{
    x: number;
    y: number;
    features: number[];
...
export interface DINOv2DetectionResult {
  isAnomalous: boolean;
  confidence: number;
  anomalies: Array<{
    patchIndex: number;
  ...
  features: DINOv2Features;
}

export class DINOv2Detector {
  ...
  async extractFeatures(
    ...
    try {
      ...
      const patches: DINOv2Features["patches"] = [];
      let baseFeatures: number[] = [];
      ...
      const patchFeatureLength = Math.min(
        patchDim,
        Math.floor((baseFeatures.length - globalDim) / numPatches)
      );

      for (let y = 0; y < patchesPerSide; y++) {
        for (let x = 0; x < patchesPerSide; x++) {
          const patchIndex = y * patchesPerSide + x;
          if (patchIndex >= numPatches) break;

          const featureStart = globalDim + (patchIndex * patchFeatureLength);
          const patchFeatures = baseFeatures.slice(
            featureStart,
            featureStart + patchFeatureLength
          );
  ...
  async detectAnomalies(
  ...
  ): Promise<DINOv2DetectionResult> {
    const features = await this.extractFeatures(imageUrl, options);
    const anomalies: DINOv2DetectionResult["anomalies"] = [];
    ...
    const patchVariances = features.patches.map(patch => {
      ...
      const variance = patch.features.reduce(
        (sum, f) => sum + Math.pow(f - mean, 2),
        0
    ...
    const stdVariance = Math.sqrt(
      patchVariances.reduce((sum, v) => sum + Math.pow(v - meanVariance, 2), 0) / patchVariances.length
    );
   ...
   */
  private calculatePatchAttention(
    patches: DINOv2Features["patches"],
    globalFeatures: number[]
  ): number[] {
  ...
  private calculateConfidence(
    anomalies: DINOv2DetectionResult["anomalies"],
