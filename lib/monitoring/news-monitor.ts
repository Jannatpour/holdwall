...
export interface NewsMonitorOptions {
  source: "rss" | "google-news" | "newsapi" | "direct";
  query?: string;
  rssUrl?: string;
  newsSiteUrl?: string;
  maxArticles?: number;
...
export class NewsMonitor {
   ...
   * Monitor Google News
   */
  private async monitorGoogleNews(
    options: NewsMonitorOptions
  ): Promise<NewsArticle[]> {
    const articles: NewsArticle[] = [];
   ...
   */
  private calculateDateFromRelative(relative: string): string {
    const now = new Date();
   ...
   */
  async monitor(options: NewsMonitorOptions): Promise<NewsArticle[]> {
  ...
  async searchBrandMentions(
    brandName: string,
    sources: Array<"rss" | "google-news" | "newsapi"> = ["google-news"],
    maxArticlesPerSource: number = 20,
    rssUrls?: string[]
  ): Promise<NewsArticle[]> {
