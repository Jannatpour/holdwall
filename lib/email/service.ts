...
export class EmailService {
  private provider: "ses" | "sendgrid" | "resend" | "smtp" | "console";
  ...
  async send(
    ...
    try {
      ...
      metrics.increment("email_sent_total", { provider: this.provider });
      metrics.observe("email_send_duration_ms", duration, { provider: this.provider });
      ...
      if (result.success) {
        ...
        metrics.increment("email_errors_total", { provider: this.provider });
  ...
  async sendApprovalRequest(
    ...
    const template = EmailTemplates.approvalRequest(data);
  ...
  async sendAlert(
    ...
    const template = EmailTemplates.alert(data);
