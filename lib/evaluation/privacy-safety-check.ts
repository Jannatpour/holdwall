...
import { PIIDetectionService } from "@/lib/signals/pii-detection";

export interface PrivacySafetyResult {
  passed: boolean;
  pii_detected: boolean;
  compliance_issues: string[];
}

export class PrivacySafetyChecker {
   ...
   */
  async check(content: string, tenantId: string): Promise<PrivacySafetyResult> {
    try {
      ...
      logger.error("Failed to check privacy safety", {
