...
export interface DefamationResult {
  passed: boolean;
  risk_level: "none" | "low" | "medium" | "high";
  issues: string[];
}

export class DefamationChecker {
  /**
   * Check for defamatory content
   */
  async check(content: string, tenantId: string): Promise<DefamationResult> {
    try {
      const issues: string[] = [];
      ...
      logger.error("Failed to check defamation", {
