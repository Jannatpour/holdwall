...
export class NeuralGraphicalModel {
   ...
   */
  private async buildProbabilityDistribution(
    node: BeliefNode,
    connectedEdges: BeliefEdge[],
    allNodes: BeliefNode[]
  ): Promise<ProbabilityDistribution> {
    const outcomes: ProbabilityDistribution["outcomes"] = [];

    // Outcome 1: Reinforcement
    const reinforcementProb = this.calculateOutcomeProbability(
      node,
      "reinforcement",
      connectedEdges.filter(e => e.type === "reinforcement")
    );
    outcomes.push({
    ...
    const neutralizationProb = this.calculateOutcomeProbability(
      node,
      "neutralization",
      connectedEdges.filter(e => e.type === "neutralization")
    );
    outcomes.push({
   ...
   */
  private calculateDistributionConfidence(
    outcomes: ProbabilityDistribution["outcomes"],
    edges: BeliefEdge[]
  ): number {
