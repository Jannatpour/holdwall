...
export interface ForecastEvent {
  nodeId: string;
  eventType: "reinforcement" | "neutralization" | "decay" | "emergence";
  probability: number;
...
export class ExplainableForecastEngine {
  ...
  ): Promise<ExplainableForecast> {
    const events: ForecastEvent[] = [];

    // Analyze each node for potential events
    for (const node of nodes) {
      const nodeEvents = await this.forecastNodeEvents(
        node,
        edges.filter(e => 
   ...
   */
  private async forecastNodeEvents(
    node: BeliefNode,
    ...
    if (reinforcementEdges.length > 0) {
      const probability = this.calculateEventProbability(
        node,
        "reinforcement",
        reinforcementEdges
      );
  ...
  private async generateExplanation(
    ...
    nodes: BeliefNode[],
    edges: BeliefEdge[]
  ): Promise<ExplainableForecast["explanation"]> {
