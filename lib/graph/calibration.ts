...
export class CalibrationEngine {
  ...
  fit(
    ...
    type: "platt" | "isotonic" = "platt"
  ): CalibrationModel {
    if (data.length < 10) {
      logger.warn("Calibration: Insufficient data, using identity mapping", { modelId, data_length: data.length });
   ...
   */
  private fitPlattScaling(data: CalibrationData[]): CalibrationModel {
   ...
   */
  private fitIsotonicRegression(data: CalibrationData[]): CalibrationModel {
    ...
    // Create bins
    const bins: Array<{ min: number; max: number; calibrated: number }> = [];
    ...
    const metrics = this.calculateMetrics(data, (score) => {
      ...
      // Extrapolate
      if (score < bins[0].min) return bins[0].calibrated;
      if (score > bins[bins.length - 1].max) return bins[bins.length - 1].calibrated;
      return 0.5;
    });
  ...
  private calculateMetrics(
    data: CalibrationData[],
    calibrateFn: (score: number) => number
  ): CalibrationModel["metrics"] {
    let brierScore = 0;
    let logLoss = 0;
   ...
   */
  calibrate(modelId: string, rawScore: number): CalibratedScore {
    const model = this.models.get(modelId);

    if (!model) {
      logger.warn("Calibration: Model not found, using identity", { model_id: modelId });
      return {
        raw_score: rawScore,
    ...
    if (model.type === "platt") {
      ...
      const bins = JSON.parse(typeof binsParam === "string" ? binsParam : "[]") as Array<{
        min: number;
        max: number;
        calibrated: number;
      }>;
      ...
      // Extrapolate
      if (bins.length > 0) {
        if (rawScore < bins[0].min) calibrated = bins[0].calibrated;
        if (rawScore > bins[bins.length - 1].max) calibrated = bins[bins.length - 1].calibrated;
      }
    }
   ...
   */
  private createIdentityModel(): CalibrationModel {
    return {
      type: "platt",
      parameters: { A: 0, B: 0 },
      metrics: {
   ...
   * Get model metrics
   */
  getModelMetrics(modelId: string): CalibrationModel["metrics"] | null {
    const model = this.models.get(modelId);
