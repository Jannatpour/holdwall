...
let secretManagerConfig: SecretManagerConfig = {
 ...
 */
function getSecretFromEnv(secretName: string): string | null {
 ...
 */
export function initializeSecretManager(config?: Partial<SecretManagerConfig>): void {
  if (config) {
...
export async function rotateSecret(secretName: string): Promise<void> {
  logger.info("Secret rotation requested", { secretName });
  ...
  switch (secretManagerConfig.provider) {
    ...
    case "env":
    default:
      logger.warn("Secret rotation not supported for environment variable provider", { secretName });
 ...
 */
async function rotateSecretInAWS(secretName: string): Promise<void> {
  const awsAccessKeyId = process.env.AWS_ACCESS_KEY_ID;
  const awsSecretAccessKey = process.env.AWS_SECRET_ACCESS_KEY;
  const region = secretManagerConfig.region || "us-east-1";
  ...
  try {
    const { SecretsManagerClient, RotateSecretCommand, DescribeSecretCommand } = await import("@aws-sdk/client-secrets-manager");
    ...
    logger.info("Secret rotation triggered in AWS Secrets Manager", {
      secretName,
    ...
    logger.error("Failed to rotate secret in AWS Secrets Manager", {
      secretName,
      error: error.message,
    });
    throw error;
 ...
 */
async function rotateSecretInVault(secretName: string): Promise<void> {
  const vaultUrl = secretManagerConfig.vaultUrl;
  const vaultToken = secretManagerConfig.vaultToken;
  ...
  try {
    ...
    logger.info("Secret rotated in HashiCorp Vault", { secretName });
  } catch (error) {
    logger.error("Failed to rotate secret in HashiCorp Vault", {
      secretName,
