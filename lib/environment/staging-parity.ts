...
export interface ParityCheckResult {
  environment: Environment;
  parity_status: "compliant" | "non_compliant" | "partial";
  checks: {
 ...
 */
export const ENVIRONMENT_CONTRACTS: Record<Environment, EnvironmentContract> = {
  development: {
  ...
  if (nodeEnv === "production") {
    ...
    if (url) {
      try {
        ...
        const stagingPatterns = ENVIRONMENT_CONTRACTS.staging.domain_patterns.staging;
        if (stagingPatterns.some((pattern) => hostname.includes(pattern))) {
          return "staging";
        }

        // Check for production patterns
        const productionPatterns = ENVIRONMENT_CONTRACTS.production.domain_patterns.production;
 ...
 */
export function getEnvironmentContract(): EnvironmentContract {
 ...
 */
export async function checkStagingParity(): Promise<ParityCheckResult> {
  ...
  const requiredVarsCheck = {
    status: (missingVars.length === 0 ? "pass" : missingVars.length === contract.required_vars.length ? "fail" : "partial") as "pass" | "fail" | "partial",
    missing: missingVars,
    present: presentVars,
    message:
  ...
  const nextAuthUrl = process.env.NEXTAUTH_URL;
  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || process.env.NEXT_PUBLIC_APP_URL;
  let domainAlignment: "pass" | "fail" | "warning" = "pass";
  ...
  const dbCheck = {
    status: (dbReachable ? "pass" : "fail") as "pass" | "fail" | "warning",
  ...
  const redisCheck = {
    status: (redisConfigured ? (redisReachable ? "pass" : "fail") : "warning") as "pass" | "fail" | "warning",
  ...
  const secretsCheck = {
    status: (missingSecrets.length === 0 && usingFallbacks.length === 0 ? "pass" : missingSecrets.length > 0 ? "fail" : "warning") as "pass" | "fail" | "warning",
  ...
  let parityStatus: "compliant" | "non_compliant" | "partial" = "compliant";
  if (failedChecks > 0) {
 ...
 */
export async function enforceStagingParity(): Promise<void> {
  const result = await checkStagingParity();

  logger.info("Staging parity check", {
  ...
  if (result.parity_status === "non_compliant") {
    ...
    if (result.environment === "production" || result.environment === "staging") {
      ...
      logger.warn("Staging parity issues in development (non-blocking)", {
        issues: result.issues,
      });
    }
  } else if (result.parity_status === "partial") {
    logger.warn("Staging parity check has warnings", {
      environment: result.environment,
      issues: result.issues,
      recommendations: result.recommendations,
    });
