...
export interface RerankOptions {
  model?: "qwen" | "cross-encoder" | "bge-reranker" | "cohere-rerank" | "voyage-rerank";
  topK?: number;
  returnDocuments?: boolean;
  // Multi-attribute ranking options
  attributes?: {
    relevance?: number; // Weight for semantic relevance (0-1)
    recency?: number; // Weight for recency (0-1)
    authority?: number; // Weight for source authority (0-1)
    popularity?: number; // Weight for popularity/engagement (0-1)
    diversity?: number; // Weight for result diversity (0-1)
  };
  diversityThreshold?: number; // Minimum distance between results for diversity
  minScore?: number; // Minimum reranking score to include
  useMultiAttribute?: boolean; // Enable multi-attribute ranking
}
...
export class Reranker {
  ...
  /**
   * Rerank documents (simplified interface for general use)
   */
  async rerank(
    query: string,
  ...
  /**
   * Rerank evidence (full interface with multi-attribute ranking)
   */
  async rerank(
    query: string,
    results: Evidence[],
    options?: RerankOptions & { useMultiAttribute: true }
  ...
  async rerank(
    query: string,
    input: Array<{ id: string; text: string; metadata?: Record<string, unknown> }> | Evidence[],
    options?: RerankOptions
  ): Promise<RerankResult[] | RerankedResult[]> {
    ...
    if (useMultiAttribute && isEvidenceArray) {
      return this.rerankWithMultiAttribute(query, input as Evidence[], options);
    }

    // Otherwise use simplified reranking
    const documents = isEvidenceArray
      ? (input as Evidence[]).map((ev) => ({
          id: ev.evidence_id,
          text: `${ev.content.raw || ""} ${ev.content.normalized || ""}`,
          metadata: ev.metadata || {},
        }))
      : (input as Array<{ id: string; text: string; metadata?: Record<string, unknown> }>);
   ...
   */
  private async rerankWithMultiAttribute(
    query: string,
    results: Evidence[],
    options?: RerankOptions
  ): Promise<RerankedResult[]> {
    ...
    const scoredResults = await Promise.all(
      results.map(async (evidence, index) => {
    ...
    for (const model of models) {
      try {
        const response = await fetch(
          `https://api-inference.huggingface.co/pipeline/sentence-similarity/${model}`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  private async rerankWithBGE(
    ...
    const response = await fetch(
      "https://api-inference.huggingface.co/pipeline/sentence-similarity/BAAI/bge-reranker-v2",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  private async rerankWithLLM(
    ...
    const ranked: RerankResult[] = (ranking.rankings || []).map((r: any) => ({
   ...
   */
  private async qwenRerankScore(query: string, content: string): Promise<number> {
    if (!this.qwenApiKey) {
   ...
   */
  private async cohereRerankScore(query: string, content: string): Promise<number> {
   ...
   */
  private async voyageRerankScore(query: string, content: string): Promise<number> {
   ...
   */
  private async bgeRerankScore(query: string, content: string): Promise<number> {
   ...
   */
  private async crossEncoderScore(
    query: string,
    content: string
  ): Promise<number> {
    if (!this.openaiApiKey) {
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a relevance scorer. Rate how relevant the content is to the query on a scale of 0.0 to 1.0. Respond with only a number.",
            },
            {
              role: "user",
   ...
   */
  private async fallbackRelevanceScore(
    query: string,
    content: string
  ): Promise<number> {
   ...
   */
  private calculateAuthorityScore(evidence: Evidence): number {
   ...
   */
  private calculatePopularityScore(evidence: Evidence): number {
...
export const RerankingService = Reranker;
