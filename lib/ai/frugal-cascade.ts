...
export interface CascadeStage {
  ...
  quality_threshold: number; // If quality < threshold, escalate to next stage
  max_tokens?: number;
  temperature?: number;
}
...
export interface CascadeConfig {
  stages: CascadeStage[];
  /** Whether to use early stopping if quality is sufficient */
  early_stop?: boolean;
  /** Quality evaluation function */
  quality_evaluator?: (response: LLMResponse, request: LLMRequest) => Promise<number>;
}
...
export interface CascadeResult {
  ...
  metadata: {
    ...
    costs_per_stage: number[];
    quality_scores_per_stage: number[];
  };
}
...
export class FrugalCascade {
  ...
  async execute(
    ...
    const stagesAttempted: number[] = [];
    const costsPerStage: number[] = [];
    const qualityScoresPerStage: number[] = [];
    let totalCost = 0;
    let escalated = false;
    ...
    for (let i = 0; i < stages.length; i++) {
      ...
      try {
        ...
        escalated = true;
        logger.info("FrugalCascade: Escalating to next stage", {
          stage: i,
          quality_score: qualityScore,
        ...
        logger.error("FrugalCascade: Stage failed", { stage: i, error, model: stage.model });
    ...
    const lastStageRequest: LLMRequest = {
      ...request,
   ...
   */
  static createDefaultConfig(taskType: "extract" | "cluster" | "judge" | "generate"): CascadeConfig {
