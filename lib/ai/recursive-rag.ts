...
export interface RecursiveRAGConfig {
  ...
  maxSubQueries: number; // Maximum sub-queries per level
  enableSynthesis: boolean; // Enable final synthesis step
  minRelevanceForSubQuery: number; // Minimum relevance to create sub-query
}
...
export class RecursiveRAG {
  private ragPipeline: RAGPipeline;
  private llmProvider: LLMProvider;
  private config: RecursiveRAGConfig;

  constructor(
    evidenceVault: DatabaseEvidenceVault,
    config?: Partial<RecursiveRAGConfig>
  ) {
  ...
  async execute(
    ...
    const queue: Array<{ query: string; depth: number; parentQuery?: string }> = [
    ...
    while (queue.length > 0) {
      ...
      if (retrieved.length > 0) {
        ...
        const needsDecomposition = await this.needsFurtherDecomposition(
          current.query,
          answer,
          retrieved,
          model
        ...
        if (needsDecomposition && current.depth < this.config.maxDepth - 1) {
          ...
          const deeperSubQueries = await this.decomposeQuery(
            current.query,
            current.depth + 1,
            model,
            current.query
    ...
    if (this.config.enableSynthesis && subQueries.length > 0) {
      totalGenerations++;
      const synthesisResponse = await this.synthesizeAnswer(
        query,
        subQueries,
        allEvidence,
        model,
...
- Necessary for answering the original query`;
    ...
    try {
      ...
      return queries.slice(0, this.config.maxSubQueries).map((q: any) => ({
      ...
      logger.warn("Recursive RAG: decomposition failed, using original query", { error });
      return [
        {
          query,
          depth,
   ...
   */
  private async needsFurtherDecomposition(
    query: string,
    answer: string,
    evidence: Evidence[],
    model: string
...
Respond with JSON:
{
  ...
  private async synthesizeAnswer(
    ...
    model: string,
    options?: { temperature?: number; maxTokens?: number }
  ): Promise<{ text: string; tokens_used: number; cost: number }> {
   ...
   */
  private buildSubQueryPrompt(query: string, evidence: Evidence[]): string {
