...
import type { BeliefNode, BeliefGraphService } from "@/lib/graph/belief";
import { ARIMAModel } from "./time-series";
import { ProphetModel } from "./time-series";
import { HawkesProcess, type HawkesEvent } from "./hawkes";
...
export interface Forecast {
  ...
  tenant_id: string;
  /** Forecast type */
  type: "drift" | "anomaly" | "outbreak" | "diffusion";
  /** Target metric */
  ...
  /** Model used */
  model: string;
  /** Evaluation score (from eval harness) */
  eval_score?: number;
  /** Evidence references */
...
export interface DriftForecast extends Forecast {
  type: "drift";
  /** Baseline value */
  baseline: number;
  /** Drift direction */
...
export interface OutbreakForecast extends Forecast {
  type: "outbreak";
  /** Probability (0-1) */
  probability: number;
  /** Trigger conditions */
...
export class ForecastService {
  constructor(
    ...
    private beliefGraph: BeliefGraphService
  ) {}

  /**
   * Generate forecasts for a tenant based on recent graph updates
   */
  async generateForecasts(tenant_id: string): Promise<Forecast[]> {
    ...
    try {
      ...
      if (strengthValues.length >= 5) {
        ...
        const driftForecast = await this.forecastDrift(
          tenant_id,
          "belief_strength",
          7, // 7-day horizon
          strengthValues
      ...
      if (limitedEvents.length > 0) {
        ...
        const outbreakForecast = await this.forecastOutbreak(
          tenant_id,
          7, // 7-day horizon
          signals
        );
  ...
  async forecastDrift(
    ...
    try {
      ...
      logger.warn("ARIMA forecast failed, trying Prophet", {
        error: error instanceof Error ? error.message : String(error),
      });
      // Fallback to Prophet
      const prophet = new ProphetModel();
      try {
        forecastResult = await prophet.forecast(baseline_data, horizon_days, {
          weekly_seasonality: true,
        });
      } catch (prophetError) {
        logger.warn("Prophet forecast failed, using exponential smoothing fallback", {
   ...
   */
  async forecastOutbreak(
    tenant_id: string,
    horizon_days: number,
    signals: Array<{ amplification: number; sentiment: number; timestamp?: number }>,
    options?: {
      use_hawkes?: boolean;
    }
  ): Promise<OutbreakForecast> {
    ...
    if (useHawkes && signals.length > 0 && signals.some((s) => s.timestamp)) {
      try {
        const hawkesEvents: HawkesEvent[] = signals
        ...
        const hawkes = new HawkesProcess();
        ...
        logger.warn("Hawkes forecast failed, falling back to logistic regression", { error });
      }
    }
    ...
    const forecast: OutbreakForecast = {
      forecast_id: `forecast-${Date.now()}`,
      tenant_id,
      type: "outbreak",
      target_metric: "outbreak_probability",
 ...
 * 
 * Every forecast must pass evaluation before being used
 */
export interface EvalHarnessGate {
  evaluate(forecast: Forecast): Promise<{
    passed: boolean;
    score: number;
    reason?: string;
  }>;
...
export class SimpleEvalHarnessGate implements EvalHarnessGate {
  async evaluate(forecast: Forecast): Promise<{
    passed: boolean;
    score: number;
    reason?: string;
  }> {
